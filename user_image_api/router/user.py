...
def add(payload: UserInsertInput, session: Session = Depends(get_db)):
    service = user.UserService(session)
    usr_id = service.add(payload)
    return UserOutput(user_id=usr_id)


@router.put(f'/v{VERSION}/update-user', status_code=200, summary="Update User")
def update(payload: UserUpdateInput, session: Session = Depends(get_db)):
    service = user.UserService(session)
